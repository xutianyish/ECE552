...
/*question 1 counters*/
static int reg_ready_q1[MD_TOTAL_REGS] = {0};
static counter_t one_cycle_q1 = 0;
static counter_t two_cycle_q1 = 0;
/*question 2 counters*/
static int reg_ready_q2[MD_TOTAL_REGS] = {0};
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "sim_inst_rate",
    ...
    /*question two code*/
    stat_reg_counter(sdb, "one_cycle_q2",
    ...
    stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
            ...
            "1 * one_cycle_q1 / sim_num_insn +2 * two_cycle_q1/sim_num_insn + 1" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);

    stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
            "CPI from RAW hazard (q2)",
            "1 * one_cycle_q2 / sim_num_insn +2 * two_cycle_q2/sim_num_insn + 1" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);
...
#include "machine.def"
	default:
	  panic("attempted to execute a bogus opcode");
        ...
        int stlll=0;
        for (i = 0; i < 3; i++) {
            if (r_in[i] != DNA && reg_ready_q1[r_in[i]] == 2) {
                stll = 2;
                break;
            }
            if (r_in[i] != DNA && reg_ready_q1[r_in[i]] == 1) {
                stll = 1;
                continue;
            }
        }
        ...
        if (r_out[0] != DNA)
            reg_ready_q1[r_out[0]] = 3;
        if (r_out[1] != DNA)
            reg_ready_q1[r_out[1]] = 3;


        for (int z = 0;z<MD_TOTAL_REGS;z++){
            reg_ready_q1[z]--;
        }
        ...
        for (i = 0; i < 3; i++) {
            if((i==0)&&(MD_OP_FLAGS(op) & F_MEM) &&(MD_OP_FLAGS(op) & F_STORE)){
                // here r0 is Data not the Address so it no need to pass x1 and x2 stage
                continue;
            }
            if (r_in[i] != DNA && reg_ready_q2[r_in[i]] == 2) {
                stlll = 2;
                break;
            }
            if (r_in[i] != DNA && reg_ready_q2[r_in[i]] == 1) {
                stlll = 1;
                continue;
            }
        }
        ...
        if (r_out[0] != DNA){
            if((MD_OP_FLAGS(op) & F_MEM) &&(MD_OP_FLAGS(op) & F_LOAD))
                reg_ready_q2[r_out[0]] = 3;
            else
                reg_ready_q2[r_out[0]] = 2;
        }
        if (r_out[1] != DNA){
            if((MD_OP_FLAGS(op) & F_MEM) &&(MD_OP_FLAGS(op) & F_LOAD))
                reg_ready_q2[r_out[1]] = 3;
            else
                reg_ready_q2[r_out[1]] = 2;
        }
